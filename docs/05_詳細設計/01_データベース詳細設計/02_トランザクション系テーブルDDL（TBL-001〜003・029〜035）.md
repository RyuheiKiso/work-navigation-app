...
-- NOTE: factories テーブルは ver1.0.0 では作成しない（04_概要設計/99 §2-5・04_概要設計/04_データ設計/99 §2-5 準拠）。
...
- **TBL-011 suspensions から resumed_at / resume_sign_id / fk_suspensions_resume_sign / ck_suspensions_resumed_after_suspended を削除し、Append-only テーブルへの後埋め列混入を解消した。再開状態は work_events.activity='work_resumed' で表現する（ADR-010 準拠）。**
- **TBL-009 evidence_files / TBL-010 measurements に created_at TIMESTAMPTZ NOT NULL DEFAULT NOW() を追加し、06_インデックス §1「全 Append-only テーブルは created_at 降順インデックス必須」との整合を確立した。**
- **DDL-040/043/044/045 の FK 参照カラム名誤りを修正した（evidence_files.evidence_id / nonconformities.nc_id / work_executions.work_execution_id / master_versions.master_version_id）。**
