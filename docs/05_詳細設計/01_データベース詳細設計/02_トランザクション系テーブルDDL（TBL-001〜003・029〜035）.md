...
**図 1: Append-only テーブルと SQL ロール分離**

![図 1 Append-only テーブルと SQL ロール分離](img/fig_dd_db_append_only_roles.svg)

> 原本: [`img/fig_dd_db_append_only_roles.drawio`](img/fig_dd_db_append_only_roles.drawio)
...
-- 制御テーブル: app_event_insert ロールに INSERT/UPDATE/DELETE を許可（idempotency_keys と同様の例外）
...
COMMENT ON TABLE case_locks IS
    'TBL-051 — case_id 単位の端末排他占有テーブル。1 case_id に同時 1 端末のみ ACTIVE 可。BAT-013 が heartbeat_at 5min 超過で EXPIRED 化。制御テーブルのため app_event_insert ロールに INSERT/UPDATE/DELETE を許可（例外）。';
...
- **TBL-051 case_locks（DDL-051）を追加確定した。heartbeat UPDATE・解放 DELETE が必要な制御テーブルとして app_event_insert ロールに INSERT/UPDATE/DELETE を許可する例外扱いとした（FR-SY-011 / ADR-009）。**
