...
> - 対象（作業実績系）: work_events・electronic_signs・evidence_files・measurements・suspensions・hash_chain_blocks・auth_logs・external_key_bindings
> - 対象（IQC/リワーク系・ADR-011）: incoming_inspection_measurements・concession_approvals・dispositions・rework_verifications・reworked_lot_labels・scrap_records・return_to_vendor_records
> - outbox_events: `INSERT, SELECT` + `UPDATE（status 列のみ）` を許可
> - incoming_inspections: `INSERT, SELECT` + `UPDATE（qc_status 列のみ）` を許可（限定可変）
>
> ```sql
> -- ロール分離 DDL（マイグレーション V000 / V20260518XXXXXX__add_hash_chain_to_iqc_tables.sql で実行）
...
> -- IQC/リワーク系 Append-only テーブル（ADR-011 対応・IQC マイグレーションで追加）
...
CREATE INDEX idx_incoming_insp_lot ON incoming_inspections (lot_id); -- IDX-021
CREATE INDEX idx_incoming_insp_supplier_status ON incoming_inspections (supplier_id, qc_status); -- IDX-022
```
...
CREATE INDEX idx_insp_meas_inspection ON incoming_inspection_measurements (inspection_id); -- IDX-023
```
...
CREATE INDEX idx_reworks_nonconformity ON reworks (parent_nonconformity_id); -- IDX-024
CREATE INDEX idx_reworks_status ON reworks (status) WHERE status NOT IN ('CLOSED_OK_RELEASE', 'CLOSED_DOWNGRADE', 'CLOSED_SCRAP', 'CLOSED_RETURN'); -- IDX-025
```
...
- **DDL-003 outbox_events の `ck_outbox_events_event_type` CHECK 制約に IQC/リワーク 9 種（'incoming_inspection' / 'incoming_inspection_measurement' / 'concession_approval' / 'rework' / 'disposition' / 'rework_verification' / 'reworked_lot_label' / 'scrap_record' / 'return_to_vendor'）を追加し、合計 18 種とした（指摘1対応）。**
- **TBL-038/040/041/044/045/047/049/050 の 8 テーブルに `qc_case_id` / `prev_hash` / `content_hash` 列と `ck_{tbl}_hash_length` CHECK 制約を追加し、IQC/リワーク品質記録の ALCOA+ Original 三層防御（DBロール制御 + 電子サイン + ハッシュチェーン）を完成させた（ADR-011 / 指摘5対応）。TBL-043/048 はハッシュチェーン除外対象。**
- **IQC/リワーク系 Append-only テーブル 7 件の REVOKE UPDATE, DELETE を NOTE セクションおよびマイグレーション手順に追加した（ADR-011 対応）。**
- **DDL-043 reworks・DDL-039 sampling_plans のインライン CREATE INDEX に IDX-024〜026 の採番コメントを付与した（指摘2対応）。**
