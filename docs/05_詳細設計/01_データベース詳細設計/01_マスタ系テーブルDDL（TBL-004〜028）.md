...
-- NOTE: IQC 拡張列（supplier_id / material_id / qc_status / rework_history_count / parent_lot_id）は
--       V20260518XXXXXX__alter_lots_add_iqc_columns.sql の ALTER TABLE で追加する。
--       TBL-036 suppliers / TBL-037 materials より後に作成されるため FK は ALTER TABLE で設定。
--       本 DDL は最終形（全列含む）を記述する。
CREATE TABLE lots (
    ...
    created_at            TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
    updated_at            TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
    -- IQC/リワーク拡張列（V20260518XXXXXX__alter_lots_add_iqc_columns.sql で ALTER TABLE 追加）
...
COMMENT ON TABLE  lots IS 'EN-021 Lot — 製造ロットマスタ。7年以上保存。lot_code は ERP/MES 連携時の外部識別子。IQC 拡張列（supplier_id / material_id / qc_status / rework_history_count / parent_lot_id）は V20260518XX ALTER TABLE で追加（指摘3対応）。';
...
CREATE INDEX idx_sampling_plans_material_supplier ON sampling_plans (material_id, supplier_id) WHERE is_active = TRUE; -- IDX-026
```
...
- **DDL-024 lots に IQC 拡張列 5 列（supplier_id / material_id / qc_status / rework_history_count / parent_lot_id）と FK 3 本・CHECK 3 本を追加し、TBLカタログ §1c の記述と DDL 本体の整合を確立した（指摘3対応）。TBL-036 suppliers / TBL-037 materials への FK は `V20260518XXXXXX__alter_lots_add_iqc_columns.sql` ALTER TABLE で設定する。**
- **DDL-039 sampling_plans のインライン `CREATE INDEX` に IDX-026 採番コメントを付与し、IDX カタログとの整合を確立した（指摘2対応）。**
