...
## 7. TBL-008 steps.required_scans（ポカヨケ照合配列）
...
```json
{
  ...
  "description": "FR-EV-013 ポカヨケ照合対象の配列。NULL は照合不要（旧来挙動）。",
  "type": "array",
  "minItems": 1,
  "items": {
    "type": "object",
    ...
    "properties": {
      ...
      "ref_id": {
        "type": "string",
        "format": "uuid",
        "description": "equipments.equipment_id または instruments.instrument_id（UUID）"
      },
      "ref_scan_code": {
        "type": "string",
        "maxLength": 64,
        "description": "ref_id 解決を行わず scan_code を直接比較する場合"
      },
      "required": {
        "type": "boolean",
        "default": true,
        "description": "false の場合は推奨照合（スキップ可）"
      },
      "label": {
        "type": "object",
        "properties": {
        ...
        "description": "UI 表示用ラベル（RequiredScansChipStack）"
      },
      "gs1_ai": {
        "type": "string",
        "pattern": "^[0-9]{2,4}$",
        "description": "期待する GS1 Application Identifier（例: '8004'）"
      }
    }
  }
}
...
- 既存 Step の required_scans は NULL とする（= 旧来の qr_scan 挙動を維持し後方互換を保つ）。
- required_scans が NULL の Step では FR-EV-004 の材料照合のみを適用する。
- required_scans が配列の Step では FR-EV-013 のマルチターゲット照合を適用し、全件合格まで step_completed を受け付けない。
...
```json
{
  ...
  "properties": {
    ...
    "scan_verifications": {
      "type": "array",
      "description": "FR-EV-013 ポカヨケ照合の結果履歴",
      "items": {
        "type": "object",
        "required": ["target", "actual_scan_value", "verified", "verified_at"],
        "properties": {
          "target": { "type": "string", "enum": ["material", "tool", "instrument"] },
          "expected_ref_id": { "type": "string", "format": "uuid", "description": "期待する ref_id（null = フリースキャン）" },
          "actual_scan_value": { "type": "string", "description": "実際のスキャン値" },
          "verified": { "type": "boolean" },
          "verified_at": { "type": "string", "format": "date-time" },
          "is_manual_input": { "type": "boolean", "default": false }
        }
      }
    }
  },
...
- **required_scans（urn:wnav:schema:required_scans:v1）と scan_verifications を追加し、ポカヨケ照合の構造化を JSON Schema レベルで強制することを確定した（FR-EV-013）。**
