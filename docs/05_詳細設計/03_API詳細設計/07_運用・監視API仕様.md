...
> **担当バイナリ方針**:
> - `/api/v1/sync/master` → **terminal-api**（子機がマスタキャッシュを取得）
> - `/api/v1/sync/outbox/inbound` → **terminal-api**（子機から親機への Outbox 送信）
> - `/api/v1/ops/outbox/dlq`・`/api/v1/ops/outbox/{id}/requeue` → **master-api**（DLQ 監視は管理コンソール）
> - `/api/v1/reports/*` → **master-api**（帳票生成は管理系）
> - `/api/v1/trace/*` → **master-api**（トレーサビリティ閲覧は管理系）
> - `/api/v1/healthz`・`/api/v1/readyz` → **terminal-api + master-api（両方）**（各バイナリが個別に公開）
> - `/api/v1/openapi.json` → **terminal-api + master-api（両方）**（terminal-api は `openapi-terminal.yaml`、master-api は `openapi-master.yaml` を返す）
...
| 担当バイナリ | master-api（DLQ 監視は管理コンソールから）|
| 認証要否 | 必須 |
...
| 担当バイナリ | master-api（DLQ 再送は管理コンソールから）|
| 認証要否 | 必須 |
| Idempotency-Key | 必須 |
| 関連 FR | FR-SY-008 |
...
| 担当バイナリ | master-api（監査ログエクスポートは管理系）|
| 認証要否 | 必須 |
| Idempotency-Key | 必須 |
| 関連 FR（レポート）| RP-002 |
...
| 担当バイナリ | terminal-api（子機がマスタキャッシュを取得するエンドポイント）|
...
| 担当バイナリ | terminal-api（子機から親機への Outbox 受信エンドポイント）|
...
| 担当バイナリ | master-api（トレーサビリティ閲覧は管理系）|
| 認証要否 | 必須 |
| 関連 FR | FR-KZ-007 |
...
| 項目 | 値 |
|---|---|
| API-ID | API-trace-002 |
| HTTP メソッド | GET |
| URL | `/api/v1/trace/backward` |
| 担当バイナリ | master-api（トレーサビリティ閲覧は管理系）|
| 認証要否 | 必須 |
| 関連 FR | FR-KZ-007 |
...
| HTTP メソッド | GET |
| URL | `/api/v1/healthz` |
| 担当バイナリ | terminal-api + master-api（両バイナリが各自のポートで個別に公開）|
| 認証要否 | 不要 |
| レート制限 | 適用外 |
| 関連 NFR | NFR-AVL-001 |
...
| 担当バイナリ | terminal-api + master-api（両バイナリが各自のポートで独立して公開）|
...
terminal-api は `openapi-terminal.yaml`（ポート 8080 のエンドポイント群）を、master-api は `openapi-master.yaml`（ポート 8081 のエンドポイント群）をそれぞれ返す。

```json
{
  "openapi": "3.1.0",
  "info": {
    "title": "WNAV Terminal API",
    "version": "1.0.0",
    "description": "作業ナビゲーションシステム REST API（terminal-api）"
  },
  "servers": [
    { "url": "https://wnav-terminal.factory.local/api/v1" }
  ],
  "paths": { "...": "..." },
  "components": { "...": "..." }
}
...
> master-api の場合は `title: "WNAV Master API"` となり、`servers[0].url` は `https://wnav-master.factory.local/api/v1`（ポート 8081）となる。
...
**本節で確定した方針**
- **sync 系（API-sync-001〜002）は `wnav_terminal_api` が担当し、ops 系（DLQ 管理）・reports 系・trace 系は `wnav_master_api` が担当することを確定した。**
- **healthz（API-system-001）・readyz（API-system-002）・openapi.json（API-system-003）は terminal-api と master-api の両バイナリが各自のポートで独立して公開することを確定した。openapi.json は terminal-api が `openapi-terminal.yaml`、master-api が `openapi-master.yaml` を返す。**
