...
| BAT-ID | バッチ名 | トリガ | 冪等性保証 |
|---|---|---|---|
| BAT-001 | Hash Chain Verification | 毎週月曜日 03:00 | 週次 1 回 |
| BAT-002 | Outbox Dispatcher | 継続・5 秒間隔 | event_id（UUID v7）|
| BAT-003 | Master Sync Puller | 60 分間隔 | sync_version 差分比較 |
| BAT-004 | PII Anonymizer | 毎日 01:00 | user_id × anonymized_at |
| BAT-005 | PostgreSQL Backup | 毎日 02:00 | date-stamped ファイル名 |
| BAT-006 | RP-001 SOP 実行記録生成 | 作業完了イベント受信時 | execution_id |
| BAT-007 | RP-003 不適合記録生成 | 不適合登録イベント受信時 | nonconformity_id |
| BAT-008 | RP-004 改善記録生成 | 改善承認イベント受信時 | kaizen_id |
| BAT-009 | RP-005 設備点検記録生成 | 点検完了イベント受信時 | inspection_id |
| BAT-010 | RP-006 集計レポート生成 | 日次 06:00・週次月曜 07:00・月次 1 日 07:00 | date + granularity |
