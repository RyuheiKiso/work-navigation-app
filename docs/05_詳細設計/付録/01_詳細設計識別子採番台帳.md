...
| IDX-032 | TBL-052 work_assignments | (target_terminal_id, status) | B-tree Partial（status IN ('pending','dispatched')）| 端末向け割当高速取得 |
| IDX-033 | TBL-052 work_assignments | received_at DESC | BRIN | 時系列検索 |
| IDX-034 | TBL-052 work_assignments | (external_system, idempotency_key) | B-tree UNIQUE | 重複受信防止 |
| IDX-035 | TBL-053 sse_dispatch_log | (assignment_id, terminal_id) | B-tree 複合 | 配信状態検索 |
...
| FNC-FE | 04_ハンディAPP詳細設計 各章 | FNC-FE-001〜019 | FNC-FE-020 |
...
- **FNC-FE-017（connectSseOrFallback）・FNC-FE-018（handleAssignmentEvent）・FNC-FE-019（startFromAssignment）は 2026-05-18 採番（MOD-FE-HA-003 AssignmentInbox 詳細設計対応）。次採番値を FNC-FE-020 に更新した。**
