...
Rust バックエンドは以下の cargo workspace 構成を採用する。

```
work-navigation-backend/
  Cargo.toml          # workspace 定義
  ├── crates/
  │   ├── wnav_api/        MOD-BE-001  axum ルータ・ミドルウェア集約
...
- **3 アプリ（FE-HA/FE-MA/FE-MC）と 1 バックエンド（BE）の役割分担を確定し、アプリ間での機能の越境を設計レベルで禁止した。**
- **Rust バックエンドの cargo workspace crate 構成（wnav_api/domain/hash_chain/db/auth/outbox/webhook の 7 crate）を確定した。**
