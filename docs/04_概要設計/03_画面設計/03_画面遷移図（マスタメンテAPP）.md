...
**図 1: マスタメンテ APP 画面遷移図**

![図 1 マスタメンテ APP 画面遷移図](img/fig_des_screen_master_flow.svg)

> 原本: [`img/fig_des_screen_master_flow.drawio`](img/fig_des_screen_master_flow.drawio)
...
**SCR-MA-004 センターペインモード切替（Step フォーム ↔ DAG フロー）について**: モード切替は intra-screen 操作であり、独立した画面遷移を発生させない。したがって TRN 識別子を発行しない（FR-MA-016）。
...
- **SCR-MA-004 のセンターペインモード切替は intra-screen 操作として TRN 採番対象外とすることを確定した。**
