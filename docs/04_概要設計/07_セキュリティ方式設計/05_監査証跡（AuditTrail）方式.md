...
|---|---|---|---|---|---|
| `audit_role`（terminal-api 専用）| 可 | 不可 | 不可 | 不可 | 不可 |
| `app_read`（master-api 専用） | 不可 | 可（audit_logs 含む）| 不可 | 不可 | 不可 |
...
| バイナリ | DB ロール | audit_logs への操作 | 備考 |
|---|---|---|---|
| terminal-api（8080） | `audit_role` | INSERT のみ（WorkEvent 書き込み）| 閲覧 API を持たない |
| master-api（8081） | `app_read` | SELECT のみ（監査照会・XES エクスポート）| 書き込み権限なし |
...
プロセスマイニング用途のために、監査イベントを XES（eXtensible Event Stream）形式でエクスポートする機能を提供する。本 API は **master-api（8081）が提供する**。端末向けの terminal-api（8080）にはこのエンドポイントは存在しない。
...
| エクスポート API | `GET /api/v1/audit/export/xes?from={date}&to={date}`（**master-api:8081 提供**）|
| アクセス権限 | quality_admin / system_admin のみ |
| 出力形式 | `.xes.gz`（gzip 圧縮） |
| DB アクセス | master-api の `app_read` pool 経由で TBL-032 を参照 |
...
- **監査ログのアクセス責務はバイナリ単位で分離する。terminal-api（8080）は `audit_role`（INSERT 専用）で WorkEvent を書き込み、閲覧 API を持たない。master-api（8081）は `app_read`（SELECT 専用）で監査照会・XES エクスポートを担当し、書き込み権限を持たない。**
- **`GET /api/v1/audit/export/xes` は master-api（8081）が提供する。端末向け terminal-api にはこのエンドポイントは存在しない。**
- **ハッシュチェーンは SHA-256 + KEY-009（日次 salt）で work_execution 単位に構成し、ハッシュチェーン検証タスク（BAT-003 相当 tokio task）が整合性を検証する。チェーン破断を検出した場合は ERR-SEC-002 → SCR-MC-008 アラート → system_admin 通知の経路でエスカレーションする。**
