...
| API コンテナ（端末向け）| `terminal-api`（Rust axum：ポート 8080）| Docker Compose サービス |
| API コンテナ（管理向け）| `master-api`（Rust axum：ポート 8081）| Docker Compose サービス |
...
| API コンテナ（端末向け）| `terminal-api`（停止中）| Docker Compose サービス | 停止中（切替後に起動）|
| API コンテナ（管理向け）| `master-api`（停止中）| Docker Compose サービス | 停止中（切替後に起動）|
...
|---|---|---|---|---|---|
| `terminal-api` | `wnav-terminal-api:x.y.z`（Rust axum）| `127.0.0.1:8080:8080`（IIS からのみアクセス可）| `./config:/app/config:ro`・`./certs:/app/certs:ro`・`./evidence-files:/app/evidence-files` | `unless-stopped` | `postgres`（healthcheck 成功後）|
| `master-api` | `wnav-master-api:x.y.z`（Rust axum）| `127.0.0.1:8081:8081`（IIS からのみアクセス可）| `./config:/app/config:ro`・`./certs:/app/certs:ro` | `unless-stopped` | `postgres`（healthcheck 成功後）|
...
**サービス起動順序**: `postgres` の healthcheck 成功後に `terminal-api` および `master-api` をほぼ同時に並列起動する。両 API サービス間に直接の依存関係はない。
...
|---|---|---|---|
| `wnav_backend` | bridge | `terminal-api`・`master-api`・`postgres` | コンテナ間通信（terminal-api → DB・master-api → DB）|
...
| 転送先（内部）— terminal-api | `/api/v1/events/*`・`/api/v1/evidence/*`・`/api/v1/sync/*`・`/api/v1/iqc/inspect/*`・`/api/v1/rework/execute/*` → `http://127.0.0.1:8080/api/v1/...` |
| 転送先（内部）— master-api | `/api/v1/sop/*`・`/api/v1/master/*`・`/api/v1/approval/*`・`/api/v1/users/*`・`/api/v1/audit/*`・`/api/v1/reports/*`・`/api/v1/ops/*`・`/api/v1/trace/*`・`/api/v1/alerts/*`・`/api/v1/capas/*`・`/api/v1/kaizen-proposals/*`・`/api/v1/iqc/judge/*`・`/api/v1/iqc/concession/*`・`/api/v1/rework/disposition/*`・`/api/v1/scrap/*`・`/api/v1/return/*` → `http://127.0.0.1:8081/api/v1/...` |
| 転送先（内部）— 認証 | `/api/v1/auth/*` → デフォルト `http://127.0.0.1:8080`（terminal-api）。接続元フロントエンドが JWT の `aud` クレームにより接続先（8080/8081）を決定してもよい。詳細は 05_外部インターフェース設計 §03 で確定する |
...
| 4. API 起動 | NODE-002 の `terminal-api` および `master-api` コンテナを並列起動 | IT 担当 | 5 分 |
...
| `./config` | Bind Mount（読取専用）| `/app/config` | `terminal-api` / `master-api` 設定ファイル（環境変数を注入）| Git 管理（機密値は除外）|
...
- Docker Compose 3 サービス（`terminal-api`・`master-api`・`postgres`）のポートマッピング（`127.0.0.1` バインド必須：8080/8081/5432）・ボリューム・再起動ポリシー（`unless-stopped`）を確定し、IIS ARR リバースプロキシ経由の URL パスルーティングによる外部公開を設計基盤とする（ADR-SYS-005）。`wnav_scheduler` は廃止し BAT ジョブを各 API の tokio task に内包する。
