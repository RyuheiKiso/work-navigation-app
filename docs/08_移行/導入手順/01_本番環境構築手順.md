...
![図 1 本番環境サーバー構成図](img/fig_mig_env_prod.svg)
...
バックエンドは2バイナリ構成であり、ポートとアクセス元 IP レンジによってセキュリティ境界を設ける。

- **terminal-api（8080）**: ハンディ端末向け。工場 LAN の IP レンジからのみ許可する。
- **master-api（8081）**: 管理・マスタメンテ向け。管理 PC の IP レンジからのみ許可する。
...
# terminal-api（axum・ハンディ端末向け・工場 LAN からのみ許可）
New-NetFirewallRule -Name "WNAV-TerminalAPI" -DisplayName "WNAV Terminal API (8080)" `
    -Direction Inbound -Protocol TCP -LocalPort 8080 `
    -RemoteAddress <工場LANのIPレンジ（例: 192.168.10.0/24）> -Action Allow

# master-api（axum・管理・マスタメンテ向け・管理 PC からのみ許可）
New-NetFirewallRule -Name "WNAV-MasterAPI" -DisplayName "WNAV Master API (8081)" `
    -Direction Inbound -Protocol TCP -LocalPort 8081 `
    -RemoteAddress <管理PCのIPレンジ（例: 192.168.1.0/24）> -Action Allow
...
- terminal-api（8080）は工場 LAN の IP レンジからのみ許可し、master-api（8081）は管理 PC の IP レンジからのみ許可することでセキュリティ境界を設けることを確定する。
...
バックエンドは2バイナリ構成であり、IIS の URL Rewrite + Application Request Routing（ARR）でパスに応じて振り分ける。

- **terminal-api（8080）**: ハンディ端末が使用するパス（events・evidence・sync・iqc/inspect・rework/execute・auth）を先にマッチさせる。
- **master-api（8081）**: 上記以外の `/api/v1/*` を全て master-api に転送する（terminal-api ルールが先にマッチするため、残余は全て master-api 向けとなる）。
...
# --- ルール 1: terminal-api（8080）向け（先にマッチさせる） ---
Add-WebConfigurationProperty `
    -PSPath "IIS:\Sites\wnav-master" `
    -Filter "system.webServer/rewrite/rules" `
    -Name "." `
    -Value @{name="Terminal API Proxy"; stopProcessing="True"}

Set-WebConfigurationProperty `
    -PSPath "IIS:\Sites\wnav-master" `
    -Filter "system.webServer/rewrite/rules/rule[@name='Terminal API Proxy']/match" `
    -Name "url" -Value "^api/v1/(events|evidence|sync|iqc/inspect|rework/execute|auth)(.*)"

Set-WebConfigurationProperty `
    -PSPath "IIS:\Sites\wnav-master" `
    -Filter "system.webServer/rewrite/rules/rule[@name='Terminal API Proxy']/action" `
    -Name "type" -Value "Rewrite"

Set-WebConfigurationProperty `
    -PSPath "IIS:\Sites\wnav-master" `
    -Filter "system.webServer/rewrite/rules/rule[@name='Terminal API Proxy']/action" `
    -Name "url" -Value "http://localhost:8080/api/v1/{R:1}{R:2}"

# --- ルール 2: master-api（8081）向け（terminal-api にマッチしない残余） ---
Add-WebConfigurationProperty `
    -PSPath "IIS:\Sites\wnav-master" `
    -Filter "system.webServer/rewrite/rules" `
    -Name "." `
    -Value @{name="Master API Proxy"; stopProcessing="True"}

Set-WebConfigurationProperty `
    -PSPath "IIS:\Sites\wnav-master" `
    -Filter "system.webServer/rewrite/rules/rule[@name='Master API Proxy']/match" `
    -Name "url" -Value "^api/v1/(.*)"

Set-WebConfigurationProperty `
    -PSPath "IIS:\Sites\wnav-master" `
    -Filter "system.webServer/rewrite/rules/rule[@name='Master API Proxy']/action" `
    -Name "type" -Value "Rewrite"

Set-WebConfigurationProperty `
    -PSPath "IIS:\Sites\wnav-master" `
    -Filter "system.webServer/rewrite/rules/rule[@name='Master API Proxy']/action" `
    -Name "url" -Value "http://localhost:8081/api/v1/{R:1}"
```

> **注意**: IIS URL Rewrite のルール評価順序は XML 定義の記述順序に従う。`Terminal API Proxy` を `Master API Proxy` より前に定義することで、ハンディ端末向けパスが正しく 8080 に転送される。`web.config` で直接ルール順を管理する場合も同様に `Terminal API Proxy` を先に記述すること。
...
- `/api/v1/(events|evidence|sync|iqc/inspect|rework/execute|auth)` パスは terminal-api（8080）へ転送することを確定する。
- 上記以外の `/api/v1/*` パスは master-api（8081）へ転送することを確定する。
- IIS ルールは terminal-api ルールを master-api ルールより先に定義することを確定する。
...
| ポート 8080 開放確認（terminal-api） | `Test-NetConnection -ComputerName localhost -Port 8080` | TcpTestSucceeded: True（terminal-api 起動後） |
| ポート 8081 開放確認（master-api） | `Test-NetConnection -ComputerName localhost -Port 8081` | TcpTestSucceeded: True（master-api 起動後） |
...
- MIG-CK-071 の疎通確認は全 9 項目（ポート 8080・8081 の両方を含む）が合格であることを IQ 合格の必要条件とすることを確定する。
...
| 0.2.0 | 2026-05-18 | バックエンド2バイナリ分割（terminal-api:8080 / master-api:8081）に伴うファイアウォール・IIS リバースプロキシ・疎通確認手順の更新 | RyuheiKiso |
