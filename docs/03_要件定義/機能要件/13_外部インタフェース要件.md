...
| IF-008 | 親機 Webhook 受信（作業指示 Push）| バックエンド ← 親機（ERP/MES）（Push）| HTTPS/REST/JSON + HMAC-SHA256 | HMAC-SHA256（KEY-010）| §9 | API-sync-003 |
| IF-009 | 端末 SSE 配信（割当通知）| バックエンド → 端末（Push）| text/event-stream | JWT RS256（aud=terminal-api）| §10 | API-sync-004 |
...
**本節で確定した方針**
- 外部インタフェースを IF-001〜IF-009 の 9 件で確定し、接続先・方向・プロトコル・縮退方針を各 IF に確定する（IF-008/009 を 2026-05-18 追加）。
...
## 9. IF-008: 親機 Webhook 受信（作業指示 Push）

### 9-1. 概要

| 項目 | 内容 |
|---|---|
| 送信元 | ERP / MES / 生産管理システム（親機） |
| 受信先 | master-api:8081 |
| 送信契機 | 製造オーダー発行・作業割当確定時（親機の任意タイミング）|
| 認証方式 | HMAC-SHA256（共有秘密 KEY-010）＋ TLS 1.3。署名ヘッダ: `X-WNAV-Signature: sha256={hex}` |
| Idempotency | `Idempotency-Key: {uuid-v7}` 必須。24h TTL（CFG-002 流用）。`(external_system, idempotency_key)` UNIQUE 制約で重複受信無害化 |

### 9-2. リクエストデータ契約（最小必須フォーマット）

```json
{
  "external_order_id":     "<MES側製造オーダーID>",
  "external_system":       "<MES識別子（例: SAP / OMRON-MES）>",
  "work_pattern_key":      "<external_key_bindingで解決するパターンキー>",
  "target_terminal_key":   "<端末の外部キー>",
  "lot_id_ext":            "<ロット外部キー（任意）>",
  "suggested_worker_key":  "<推奨作業員外部キー（任意）>",
  "suggested_equipment_key": "<推奨設備外部キー（任意）>",
...
| HTTP ステータス | 意味 | 対処 |
|---|---|---|
| 202 Accepted | 受理・キュー登録完了 | 正常 |
| 401 Unauthorized | HMAC 署名不正（ERR-VAL-032）| MES 側鍵設定確認 |
| 409 Conflict | 重複受信（Idempotency-Key 一致）| MES 側は 200 相当として処理可 |
| 422 Unprocessable Entity | external_key 解決不能（ERR-BIZ-027）| IT 担当が external_key_binding を設定 |
| 503 Service Unavailable | 一時障害 | MES 側は指数バックオフでリトライ可 |

### 9-4. エラー処理と縮退

| エラー種別 | 処置 |
|---|---|
| HMAC 署名不正 | 即時 401 返却。リクエストを DB に保存しない |
| external_key 解決不能 | 422 返却。`pending_resolution` ステータスの行を DB に INSERT し IT 担当に通知（ERR-BIZ-027）|
| DB 障害（INSERT 失敗）| 503 返却。MES 側リトライを期待 |

**本節で確定した方針**
- IF-008 を HMAC-SHA256 認証・Idempotency-Key 必須・202 Accepted（受理即応答）方式として確定する。
- external_key 解決不能時も `pending_resolution` 行を作成して IT 担当に通知する設計を確定する。これによりキー未設定の割当が無言でロストすることを防ぐ。

---

## 10. IF-009: 端末 SSE 配信（割当通知）

### 10-1. 概要

| 項目 | 内容 |
|---|---|
| 配信先 | ハンディ端末（React Native）|
| 配信元 | terminal-api:8080 |
| 認証方式 | JWT Bearer（aud=terminal-api、`terminal_id` クレーム必須）|
| MIME | `text/event-stream` |
| 接続単位 | 端末単位（terminal_id）1端末 = 1 SSE コネクション |

### 10-2. イベント仕様

| event type | 発火タイミング | data フィールド |
|---|---|---|
| `assignment.created` | 新規割当がキューに追加されたとき | assignment_id, work_pattern_id, lot_id, due_at, priority |
| `assignment.cancelled` | 割当が cancelled/expired になったとき | assignment_id |
| `assignment.updated` | due_at / priority が変更されたとき | assignment_id, due_at, priority |
| `keepalive` | 25 秒ごと（CFG-029）| — |

### 10-3. 再接続規約

端末は `Last-Event-ID` ヘッダに最後に受信した `assignment_id`（UUID v7）を設定して再接続する。サーバーは `received_at` > `Last-Event-ID` の timestamp の pending/dispatched 行を昇順で一括配信してから、以降の新規分をリアルタイム配信する。

### 10-4. 縮退方針

| 障害 | 縮退動作 |
|---|---|
| SSE 接続不可（ネットワーク障害・サーバー再起動）| 端末は API-sync-005（GET /api/v1/work-assignments?status=pending）を 30 秒間隔でポーリングする |
| JWT 有効期限切れ | 端末はトークンを更新してから再接続する |
| BAT-014（retry scheduler）| sse_dispatch_log の sent 未確認行を 1 分周期でリトライ配信 |

**本節で確定した方針**
- IF-009 を JWT 認証・terminal_id クレーム・Last-Event-ID 再接続方式として確定する。
- SSE 不可時の Pull フォールバック（API-sync-005）を設計必達とし、リアルタイム性と信頼性を両立する。
