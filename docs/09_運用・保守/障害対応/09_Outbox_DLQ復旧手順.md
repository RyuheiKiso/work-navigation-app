...
最終更新: 2026-05-18 | 管理者: system_admin | 根拠要件: ALERT-001
...
```bash
# DLQ 統計サマリー
curl -fsS http://localhost:8080/api/v1/ops/dlq/stats \
...
```bash
# Outbox Worker が稼働中であることを確認
curl -fsS http://localhost:8080/api/v1/ops/outbox/worker/status \
...
watch -n 30 'curl -fsS http://localhost:8080/api/v1/ops/dlq/stats \
...
```bash
# 廃棄対象の特定
curl -fsS "http://localhost:8080/api/v1/ops/dlq/candidates?type=discard" \
  -H "Authorization: Bearer ${SYSTEM_ADMIN_JWT}" | jq .
```
...
```bash
# 廃棄実行（quality_admin 承認後）
curl -fsX DELETE "http://localhost:8080/api/v1/ops/dlq/{DLQ_ID}" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer ${SYSTEM_ADMIN_JWT}" \
