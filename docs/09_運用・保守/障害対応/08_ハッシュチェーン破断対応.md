...
最終更新: 2026-05-18 | 管理者: system_admin | 根拠要件: ERR-DB-003 / MET-006 / OPS-062
...
| トリガー | 確認方法 |
|---|---|
| ERR-DB-003 が API ログに出現 | `docker compose logs api | grep ERR-DB-003` |
| MET-006（hash_chain_error_total）> 0 | Prometheus クエリ: `hash_chain_error_total > 0` |
| BAT-003（ハッシュチェーン検証）が失敗 | `batch_job_status.last_status = 'FAILED' WHERE job_id='BAT-003'` |
...
# API ログの保全
docker compose logs api --since 24h > ${WAL_BACKUP_DIR}/api_log_$(date +%Y%m%d_%H%M%S).log
...
```bash
# SCR-MC-008 承認フローの開始（API 経由）
curl -fsX POST http://localhost:8080/api/v1/change-requests \
...
# 承認確認
curl -fsS "http://localhost:8080/api/v1/change-requests/{CR_ID}" \
...
INSERT INTO step_results (
  operator_id, step_id, result, recorded_at,
  is_correction, original_record_id, correction_reason,
  approver_system_admin, approver_quality_admin
)
VALUES (
  '{OPERATOR_ID}', '{STEP_ID}', 'pass', '{RECORDED_AT}',
  TRUE, {ORIGINAL_ID}, 'ハッシュチェーン破断後の補正: INC-YYYY-NNN',
  'system_admin', 'quality_admin'
);
```

**本節で確定した方針**
- **DELETE・UPDATE は禁止する。補正は新規レコード追加のみで実施する（ALCOA+ Original）。**
- **補正レコードには is_correction=TRUE・original_record_id・correction_reason の 3 項目を必須とする。**
