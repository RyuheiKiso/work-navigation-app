...
```bash
# .env.example
# このファイルをコピーして .env を作成し、値を設定すること
# cp .env.example .env

# ===== バックエンド（BE）=====
# 接続先 PostgreSQL（3 ロール分）
WNAV_BE_DATABASE_URL_WRITE=postgres://wnav_write:CHANGE_ME@localhost:5432/wnav_dev
WNAV_BE_DATABASE_URL_EVENT=postgres://wnav_event:CHANGE_ME@localhost:5432/wnav_dev
WNAV_BE_DATABASE_URL_READ=postgres://wnav_read:CHANGE_ME@localhost:5432/wnav_dev
...
# ===== マスタメンテ（FE_MA）=====
WNAV_FE_MA_API_BASE_URL=http://localhost:8080/api/v1
...
# パスワード等のシークレットを設定する（vi/nano 等で編集する）
# CHANGE_ME となっている値を実際の値に変更すること
vi .env

# .env が .gitignore に含まれていることを確認する
grep ".env" .gitignore

# シークレット取得先（開発環境）
# - DB パスワード: Docker Compose の secrets/ または手動で設定
# - JWT 公開鍵: src/backend/scripts/generate_jwt_keys.sh で生成
```

**本節で確定した方針**
- **`.env.example` のみをバージョン管理に含め、`.env` を `.gitignore` で除外する。**
- **全変数名を `WNAV_<SCOPE>_<KEY>` 形式で統一し、変数の目的が名前から分かるようにする。**
- **`CHANGE_ME` プレースホルダーを使用し、未設定のシークレットを視覚的に識別しやすくする。**
...
# バックエンド + フロントエンドを同時起動する
# ターミナル 1: バックエンド
cd src/backend && cargo watch -x run
...
**本節で確定した方針**
- **`GET /healthz` エンドポイントの正常レスポンスを起動確認の必須条件とする。**
- **`cargo watch -x run` を使用してバックエンドのファイル変更監視と自動再起動を行う。**
