...
- **バックエンド（API）**: WSL2 + Docker コンテナで稼働する Rust/axum サーバー（ポート 8080）
...
Active-Standby 構成として、本番インスタンスに障害が発生した場合にサービスを継続する縮退モードを定義する。縮退モードでは backend コンテナが停止した状態でも、ハンディ APP は Offline-First 原則に基づきローカル SQLite から手順ナビゲーションを継続し、作業ログを Outbox に蓄積する。backend 復旧後に Outbox が自動同期される。

**本節で確定した方針**
- **Active-Standby 構成を採用する**: backend 停止中もハンディ APP はローカル動作を継続する
- **縮退モードを定義する**: backend 未到達時は Emergency Mode に遷移し、最終同期時刻を常時表示する
...
各環境は `WNAV_<SCOPE>_*` 環境変数で設定を切り替える（詳細は 12 章参照）。`local` と `dev` は `.env` ファイルを使用し、`staging` と `prod` は Windows DPAPI または GitHub Actions Secrets から注入する。

**本節で確定した方針**
- **4 環境（local / dev / staging / prod）を定義する**: 環境間のコンフィグ差異は環境変数のみで吸収する
...
backend（Rust/axum API サーバー）のデプロイ手順を以下に示す。

### 3.1 イメージ取得

```bash
# ghcr.io からリリースイメージを取得する
docker pull ghcr.io/ryuhei-kiso/wnav-api:v1.0.0

# タグのダイジェストを確認して改竄がないことを検証する
docker image inspect ghcr.io/ryuhei-kiso/wnav-api:v1.0.0 \
  --format '{{.RepoDigests}}'
```
...
IMAGE_TAG=v1.0.0 docker compose up -d backend
...
WSL2 環境では systemd が利用可能な場合、Docker デーモンを systemd unit として管理する。

```ini
# /etc/systemd/system/wnav-backend.service
[Unit]
Description=WNAV Backend Container
...
ExecStop=/usr/bin/docker compose down backend
Restart=on-failure
...
sudo systemctl enable wnav-backend.service
sudo systemctl start wnav-backend.service
```

### 3.4 ヘルスチェック確認

```bash
# backend のヘルスチェックエンドポイントを確認する（DB 疎通確認含む）
...
ヘルスチェックが `200 OK` を返さない場合はデプロイを中断し、ロールバック手順（§ 9 参照）を実行する。
...
docker compose down backend
IMAGE_TAG=v0.9.0 docker compose up -d backend
...
**本節で確定した方針**
- **ヘルスチェック通過をデプロイ完了条件とする**: `GET /healthz` が `200 OK` を返すまでデプロイ完了と見なさない
- **ロールバックは IMAGE_TAG 指定で即時実行可能とする**: イメージは ghcr.io に 30 日間保持する
- **systemd unit を定義する**: サーバー再起動時の自動起動を保証する
