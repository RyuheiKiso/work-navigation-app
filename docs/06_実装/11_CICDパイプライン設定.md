...
図: fig_cicd_workflow_main（img/ 配下）を参照
...
env:
  REGISTRY: ghcr.io
  IMAGE_NAME: ${{ github.repository_owner }}/wnav-api
...
|---|---|---|---|
| Docker image (backend) | ghcr.io/ryuhei-kiso/wnav-api | 30 日（`latest` は永続） | `v{version}` / `latest` |
...
```bash
# Docker イメージのタグ命名例
ghcr.io/ryuhei-kiso/wnav-api:v1.0.0    # バージョンタグ（変更不可）
ghcr.io/ryuhei-kiso/wnav-api:v1.0      # マイナーバージョンタグ（パッチで更新）
ghcr.io/ryuhei-kiso/wnav-api:latest    # 最新安定版（main ブランチのみ）
```
...
図: fig_cicd_workflow_release（img/ 配下）を参照
...
4. Docker イメージをビルドして ghcr.io に push する
5. staging 環境に自動デプロイして動作確認する
