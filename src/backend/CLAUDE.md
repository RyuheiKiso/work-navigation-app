...
| ロール | 権限 | 接続プールの用途 |
|---|---|---|
| `app_write` | SELECT / INSERT / UPDATE（マスタテーブルのみ） | マスタ CRUD |
| `app_event_insert` | INSERT のみ（作業ログテーブル） | 作業ログ記録 |
| `app_read` | SELECT のみ（全テーブル） | Audit Trail 照会・ダッシュボード |

接続プールは用途別に分離する。`app_event_insert` プールから SELECT を発行してはならない。
...
src/backend/
  ...
  src/
    lib.rs
    main.rs
    api/              # axum ハンドラ・ルーティング定義
    domain/           # ビジネスロジック（イベント生成・ハッシュチェーン）
    db/               # sqlx クエリ・接続プール設定
    auth/             # JWT 検証・RBAC
    webhook/          # Webhook 配信・HMAC 署名
    middleware/       # Tower ミドルウェア（認証・冪等性・レート制御・相関 ID）
    error.rs          # thiserror エラー型定義
