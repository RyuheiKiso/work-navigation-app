#!/bin/bash
# backup.sh — 日次フルバックアップスクリプト（BAT-001）
# 権威ドキュメント:
...
#
# 実行スケジュール: 毎日 02:00 JST（BAT-001）
# Standby rsync:   毎日 02:15 JST（BAT-001 完了後に STANDBY_HOST が設定されている場合のみ）
...
# 保持する世代数（7 世代 = 7 日分を保持し、8 世代目を削除する）
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] BAT-001 バックアップ開始"
echo "  保存先: ${ENC_FILE}"
...
# Step 4: 7 世代を超えた古いバックアップを削除する（8 世代目を削除する）
# ls -t で新しい順に並べて、8 件目以降を削除する
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] 古いバックアップを削除中（${RETENTION_GENERATIONS} 世代保持）..."
ls -t "${BACKUP_DIR}"/work_nav_*.dump.gz.enc 2>/dev/null | \
    tail -n +$((RETENTION_GENERATIONS + 1)) | \
    xargs -r rm -f

# 対応するチェックサムファイルも削除する
ls -t "${BACKUP_DIR}"/work_nav_*.dump.gz.enc.sha256 2>/dev/null | \
    tail -n +$((RETENTION_GENERATIONS + 1)) | \
    xargs -r rm -f

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] BAT-001 バックアップ完了"
echo "  ファイル: ${ENC_FILE}"
